...
async function extractDescription(page) {
    ...
    try {
        ...
        for (const selector of metaSelectors) {
            try {
                ...
                if (metaDesc && metaDesc.length > MIN_PART_LENGTH / 3) {
                    ...
                    if (metaDesc.length > 250 && descriptionParts.size >=1) break; // Si una meta es muy buena, puede ser suficiente
                }
        ...
        if (currentDescTextLength < MIN_PART_LENGTH * 1.5 && currentDescTextLength > 0) { 
            // No añadir el título si ya tenemos algo sustancial
        } else if (currentDescTextLength === 0) { // Solo añadir título si no hay absolutamente nada más
...
async function extractPhones(page) {
    ...
    try {
        ...
        while ((phoneMatch = phoneRegexBroad.exec(bodyText)) !== null) {
            ...
            if (numericCandidate.startsWith('+') ? (numericCandidate.length >= 9 && numericCandidate.length <= 18) : (numericCandidate.length >= 7 && numericCandidate.length <= 16)) {
                // Evitar secuencias numéricas que parecen más IDs o códigos postales largos
                if (candidate.replace(/[\s().+-]/g, '').length < 15 && /\D/.test(candidate) || candidate.length < 15) { // Que tenga algún no-dígito o sea corto
    ...
    if (potentialPhones.size > 0) {
        const bestPhone = Array.from(potentialPhones)
            .map(p => ({ original: p, numeric: p.replace(/[^\d]/g, '') })) // Guardar original
...
async function runEnrichment(targetUrl) {
    ...
    try {
        ...
        page.on('request', (req) => {
            ...
            if (['stylesheet', 'font', 'image', 'media'].includes(resourceType)) { // Dejamos 'other' (XHR/fetch) y 'script'
                req.abort();
            } else {
                req.continue();
            }
        ...
        if (!output.telefono) {
            ...
            try {
                const currentPageUrlForEval = page.url(); // Definir ANTES de page.evaluate
                console.error(`[API-ENRICH-DEBUG] URL actual antes de buscar enlace de contacto: ${currentPageUrlForEval}`);
                
                const contactPageUrl = await page.evaluate((currentPUrl) => { // El argumento aquí se llama currentPUrl
                    ...
                    for (const link of links) {
                        ...
                        // Usar currentPUrl (el argumento pasado) en lugar de page.url()
                    ...
                    return null;
                }, currentPageUrlForEval); // Pasar currentPageUrlForEval como el segundo argumento
...
app.get('/enrich', async (req, res) => {
    ...
    console.log(`[API] Solicitud de enriquecimiento recibida para: ${urlToScrape}`);
    try {
        ...
        console.error(`[API-FATAL] Error fatal en endpoint /enrich: ${e.message.split('\n')[0]}`);
        res.status(500).json({ url_procesada: urlToScrape, error: 'Error interno del servidor al procesar la solicitud.', details: e.message.substring(0,100), correos: [], descripcion: null, telefono: null });
    }
});

app.get('/', (req, res) => {
    res.send('API de Enriquecimiento funcionando OK!');
});

app.use((err, req, res, next) => {
  console.error("[EXPRESS-ERROR-HANDLER]", err.stack);
  // Evitar enviar respuesta si ya se envió una
  if (!res.headersSent) {
